...
{	
	...
	TIM_OC3Init(TIM4, &TIM_OCInitStructure);                      //根据T指定的参数初始化外设TIM4OC1
	TIM_OC3PreloadConfig(TIM4, TIM_OCPreload_Enable);             //使能TIM14在CCR1上的预装载寄存器

	TIM_OC4Init(TIM4, &TIM_OCInitStructure);                      //根据T指定的参数初始化外设TIM4OC1
...
{
	 TIM_SetCompare3(timx,setvalue);
}

void Set_Pwm4(TIM_TypeDef *timx, uint16_t setvalue)
{
	 TIM_SetCompare4(timx,setvalue);
}
